...
func ParseCLIArguments() *constants.ParamsConfig {
	...
	flag.StringVar(&config.AwsSecretCredentials, "awsSecretCredentials", "", "This is an optional parameter and it would only be passed when the artifactsRegistryProvider is aws and it represents the credentials for the AWS ECR registry.")
	flag.StringVar(&config.AzureVaultCredentials, "azureVaultCredentials", "", "This is an optional parameter and it would only be passed when the artifactsRegistryProvider is azure and it represents the credentials for the Azure ACR registry.")
